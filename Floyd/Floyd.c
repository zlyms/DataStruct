 ...
 *   - path: 路径矩阵
 */
void iniDisPath(ALGraph* alg, int dis[][MAXSIZE], int path[][MAXSIZE])
{
	if (alg == NULL)
	{
		return;
	...
	for (int i = 0; i < alg->vnum; i++)
	{
		...
		while (p)
		{
			dis[i][p->adjvex] = p->arcValue; // 将边的权值赋给距离矩阵
			p = p->NextArc;
		}
		for (int j = 0; j < alg->vnum; j++)
		{
			if (dis[i][j] == 0)
			{
				dis[i][j] = 9999; // 如果没有直接连接，则赋值为无穷大
			}
			if (i == j)
			{
				dis[i][j] = 0; //自己到自己的距离为0
			}
			path[i][j] = -1; // 初始化路径矩阵为-1
		}
	}
}

/*
 * 使用 Floyd 算法计算图的最短路径和路径矩阵
 * 参数：
 *   - alg: 指向图的指针
 */
void Floyd(ALGraph* alg)
{
	int dis[MAXSIZE][MAXSIZE] = { 0 }; // 距离矩阵
	int path[MAXSIZE][MAXSIZE] = { 0 }; // 路径矩阵
	iniDisPath(alg, dis, path); // 初始化距离矩阵和路径矩阵
	for (int k = 0; k < alg->vnum; k++)
	{
		for (int i = 0; i < MAXSIZE; i++)
		{
			for (int j = 0; j < MAXSIZE; j++)
			{
				if (dis[i][j] > dis[i][k] + dis[k][j])//如果通过中转点到目标顶点距离更短则更新路径和距离
				{
					path[i][j] = k; // 更新路径矩阵
					dis[i][j] = dis[i][k] + dis[k][j]; // 更新距离矩阵
				}
			}
		}
	}
