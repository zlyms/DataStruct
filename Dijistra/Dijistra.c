...
/*
 * 使用 Dijkstra 算法计算从顶点 v 到图中其他顶点的最短距离及最短路径
 * 参数：
 *   - alg: 指向图的指针
 *   - v: 开始遍历的顶点
 * 注意事项：
 *   - 此函数假设图中不存在负权边，否则无法得到正确的最短距离
 */
void Dijistra(ALGraph* alg, vextype v)
{
	int t = Getindex(alg, v); // 获取顶点 v 在顶点集合中的索引
	char final[MAXSIZE] = { 0 }; // 记录已确定最短路径的顶点
	int path[MAXSIZE] = { 0 }; // 记录最短路径的前驱顶点
	int dis[MAXSIZE] = { 0 }; // 记录顶点到起始顶点 v 的最短距离
	for (int i = 0; i < MAXSIZE; i++)
	{
		path[i] = -1; // 初始化路径为 -1，表示未知路径
		dis[i] = 9999; // 初始化距离为一个较大的值，表示未知距离
	}
	final[t] = 1; // 将起始顶点标记为已确定最短路径
	dis[t] = 0; // 设置起始顶点到自身的距离为 0

	while (JudgeVisted(final, alg)) // 当存在未确定最短路径的顶点时循环执行
	{
		ArcNode* p = alg->vertices[t].firstArc; // 获取顶点 t 的第一个邻接顶点
		int x = 9999; // 用于记录最短路径长度
		while (p)
		{
			if (p->arcValue + dis[t] < dis[p->adjvex] && final[p->adjvex] == 0) // 如果通过顶点 t 到达顶点 p->adjvex 的路径更短
			{
				dis[p->adjvex] = p->arcValue + dis[t]; // 更新从顶点 v 到顶点 p->adjvex 的最短距离
				path[p->adjvex] = t; // 记录从顶点 v 到顶点 p->adjvex 的最短路径的前驱顶点
			}
			p = p->NextArc; // 访问下一个邻接顶点
		}
		for (int i = 0; i < alg->vnum; i++)
		{
			if (dis[i] < x && final[i] == 0) // 找到未确定最短路径的顶点中距离最小的顶点
			{
				x = dis[i];
				t = i; // 更新 t 为距离最小的顶点索引
			}
		}
		final[t] = 1; // 将顶点 t 标记为已确定最短路径
	}
}
int main()
{
